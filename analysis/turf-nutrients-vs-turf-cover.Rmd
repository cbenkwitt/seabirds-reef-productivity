...
#turf cover----
turf_cover_dat<-read.csv("../data/seychelles_turf_cover.csv")
str(turf_cover_dat)

##turf isotopes----
turf_iso_dat<-read.csv("../data/seychelles_isotope_data.csv")%>%
...
#get mean turf n15 per site, then combine with turf cover data:
turf_n15_site<-
  turf_iso_dat%>%
  group_by(Island, Site)%>%
  summarize(mean_turf_n15 = mean(N15, na.rm = TRUE),
            se_turf_n15 = sd(N15, na.rm = TRUE)/sqrt(length(N15)))%>%
  ungroup()
turf_n15_site

turf_cover_iso_dat<-left_join(turf_cover_dat, turf_n15_site,
               by = c("Island", "Site"))

turf_cover_iso_dat

#save for use elsewhere----
#write.csv(turf_cover_iso_dat, "../data/seychelles_turf_cover_isotopes_by_transect.csv")
...
turf_cover_iso_dat_cs<-
  turf_cover_iso_dat%>%
  mutate(turf_n15_cs = base::scale(mean_turf_n15, center = TRUE, scale = TRUE),
         turf_n15_c = base::scale(mean_turf_n15, center = TRUE, scale = FALSE),
...
turf_cover_nutrients_brms<-
  brm(
 log_turf_cover~turf_n15_c + (1|Island),
  data = turf_cover_iso_dat_cs, 
  iter = 3000, warmup = 1000, chains = 4, cores = 4,
   control = list(adapt_delta = 0.999, max_treedepth = 15), 
      file = "../outputs/brms/turf_cover_nutrients_brms") #all good
print(turf_cover_nutrients_brms) #worked
...
hypothesis(turf_cover_nutrients_brms, "turf_n15_c>0") 
  ...
  #(turf_n15_c) > 0       	-0.3  	0.33    	-0.84     	0.22	      0.18                    	0.15
...
# -0.301251       	-0.9531769	0.3893436		0.95	median	hdi
...
#0.739892	0.3179007	1.307303	0.95	median	hdi

#so for each one unit increase in turf nutrients, turf cover decreases by a factor of 0.738

#equivalent to: 
1-0.739892 #0.260108 that's a 26.0% decrease
1-0.3179007 #0.6820993 that's a 68.2% decrease
1.307303-1 # 0.307303 that's a 30.7% increase
...
p_cover_total<-partialize(turf_cover_nutrients_brms, vars= "turf_n15_c",  data = turf_cover_iso_dat_cs)



#back-transform growth, centered/scaled n15:----
#get data for un-scaling and centering:
mean(turf_cover_iso_dat_cs$mean_turf_n15, na.rm=TRUE) #4.653333
...
#  scale_x_continuous(limits = c(0,5), breaks=seq(0, 5,2.5))+
  theme(panel.grid.major = element_blank(), # remove gridlines
        panel.grid.minor = element_blank(), #remove gridlines
        strip.background = element_blank(), 
        legend.position = "none",  
...
# save panel----
#ggsave(filename = "../outputs/figures/figure3_turf_cover_by_n15.png", 
#       plot = turf.cover.plot.with.inset,  width = 11,       height = 7.5,   units = "cm",     dpi = 300)
...
#ggsave(filename = "../outputs/figures/figure3_combined.png", 
#       plot = fig3,
#       width = 22, 
#       height = 10,
#       units = "cm",
...
ggsave(filename = "../outputs/figures/figure3_combined.pdf", 
       plot = fig3,
       width = 25, 
       height = 22,
       units = "cm",
