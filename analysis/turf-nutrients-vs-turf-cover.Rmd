...
print(turf_cover_nutrients_brms) #worked - only 1 divergent trans so okay
...
hypothesis(turf_cover_nutrients_brms, "turf_n15_c>0") 
  ...
  #(turf_n15_c) > 0       -0.39	      0.42	  -1.06	      0.3	          0.19                	0.16	
...
# -0.395345	-1.265228	0.4460393			0.95	median	hdi
...
#0.6734477	0.2167097	1.402084		0.95	median	hdi

#so for each one unit increase in turf nutrients, turf cover decreases by a factor of 0.738

#equivalent to: 
1-0.6734477 #0.3265523 that's a 32.7% decrease
1-0.2167097 #0.7832903 that's a 78.3% decrease
1.402084-1 # 0.402084 that's a 40.2% increase
...
#ggsave(filename = "../outputs/figures/figure3_combined_r1.png", 
       plot = fig3,
       width = 22, 
      height = 10,
      units = "cm",
...
#ggsave(filename = "../outputs/figures/figure3_combined_r1.pdf", 
      plot = fig3,
       width = 25, 
       height = 22,
      units = "cm",
...
##re-plot for final figure (because alternative dag doesn't change this part) - (with exposure)
```{r}


turf_cover_post_plot_alt<-
turf_cover_nutrients_brms %>%
  spread_draws(b_turf_n15_c) %>%
...
#ggsave(filename = "../outputs/figures/fig_turf_cover_post_plot_alt.pdf", plot = turf_cover_post_plot_alt,  width = 8,   height = 5,  units = "cm", dpi = 300)
```