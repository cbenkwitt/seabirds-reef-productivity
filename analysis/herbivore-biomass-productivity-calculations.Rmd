...
#need to keep FG.fine even when 0 for transect FG.finer_New
sey_herb_biomass_transect_fg<-
   sey_herb_dat%>%
    select(Island, Site, Transect, FG.fine, biomass.kgha)%>%
    droplevels()%>%
...
sey_herb_productivity_transect_fg<-
  datagr%>%
    select(Island, Site, Transect, FG.fine, Biomass_gained_1_kg_ha)%>%
    droplevels()%>%
...
sey_herb_bio_prod_transect_fg<-left_join(sey_herb_biomass_transect_fg, sey_herb_productivity_transect_fg, 
                                      by = c("Island", "Site", "Transect", "FG.fine"))
