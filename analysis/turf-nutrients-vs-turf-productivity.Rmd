...
#(turf_n15_c) > 0        0.04	      0.03	    0	        0.09	        14.75	                0.94	  
...
# 0.04407482  	-0.01498565	  0.1029343			  0.95	median	hdi
...
#hypothesis              est      error     lower     upper       evidence ratio        posterior probability
#(turf_n15_c) > 0       0.05      	0.03    	0.01    	0.1	        31.79	                0.97	*
...
turf_pp_nutrients_total_brms %>%
  ...
  median_hdi(b_turf_n15_c)
#b_log_seabird      lower       upper       width   point     interval
# 0.05556064	-0.0004065733	0.1137147	    0.95    	median	hdi
...
#hypothesis              est      error     lower     upper       evidence ratio        posterior probability
# (turf_n15_c) > 0        0.03	     0.02	    0.01	0.06	          46.06                 	0.98	*
...
#b_log_seabird      lower       upper       width   point     interval
#0.03454716	      0.001523137	  0.06772498				  0.95	median	hdi
#.034 increase in proportional growth



###plot it------
#plot partialized residuals-----
me_n15_prop<-conditional_effects(prop_turf_pp_nutrients_brms, prob = .75, effects = 'turf_n15_c', plot = FALSE)[[1]]

#extract partialized residuals
p_n15_prop<-partialize(prop_turf_pp_nutrients_brms, vars= "turf_n15_c",  data = turf_pp_cage_all_dat_prop_c)


#get data for un-centering:
mean(turf_pp_cage_all_dat_prop_c$Turf_N15, na.rm=TRUE) #4.67381
...
turf_pp_n15_prop_part_plot<-
p_n15_prop %>%
  ggplot(aes(x = Turf_N15, y = Prop_turf_growth_day)) +
     #  geom_jitter(color = "#EE7733", fill = "#EE7733", alpha = 0.4, size = 3, width=.05, height = 0) + #, size = .8  
     geom_point(color = "#EE7733", fill = "#EE7733", alpha = 0.4, size = 3) + #, size = .8  width=.07, height = 0,
    geom_ribbon(data = me_n15_prop_unscale, aes(ymin = lower__, ymax=upper__), alpha = .2, fill = "#EE7733")+
    geom_line(data = me_n15_prop_unscale, aes(x = Turf_N15, y = estimate__), lwd = 1.2, colour = "#EE7733") + ##scale line thickness to PP - >0.9 = 1.2 (original thickness)
xlab(expression(paste("Turf ", delta^15, "N"))) + 
  ylab(bquote(Turf~productivity~(proportional~growth~day^-1)))+
  theme_bw()+
    guides(size = "none", colour = "none",  fill = guide_legend(title="Confidence level")) + 
    theme(panel.grid.major = element_blank(), # remove gridlines
        panel.grid.minor = element_blank(), #remove gridlines
...
##combine and save---------
##make inset plots, then combine and save:
turf.pp.n15.plot.with.inset.prop <-
  ggdraw() +
  draw_plot(turf_pp_n15_prop_part_plot) +
  draw_plot(turf_pp_prop_post_plot, x = 0.1, y = .64, width = .35, height = .35)
turf.pp.n15.plot.with.inset.prop
...
#ggsave(filename = "../outputs/figures/turf_pp_post_plot_alt.pdf", plot = turf_pp_post_plot_alt,  width = 8,   height = 5,  units = "cm", dpi = 300)
```
